  ...
  if (doc.HasMember("cups")) {
    ...
    if (cups_section.HasMember("use_nwi")) {
      ...
      if (cups_section.HasMember("trigger_association")) {
        ...
        cups_.trigger_association =
            cups_section["trigger_association"].GetBool();
      }
      if (cups_section.HasMember("up_nodes_selection")) {
        ...
        for (RAPIDJSON_NAMESPACE::SizeType i = 0; i < nodes_section.Size();
             i++) {
...
#define PACKAGE_VERSION "UNKNOWN PACKAGE VERSION"
  ...
  void pgw_config::Display() {
    ...
    Logger::pgwc_app().info(
        "    workers ..........: %u", gtpv2c_.worker_threads);
    Logger::pgwc_app().info(
    ...
    //  Logger::pgwc_app().info("    APN AMBR UL ..........: %lu  (Kilo
    //  bits/s)",
    //                          pcef.apn_ambr_ul);
    //  Logger::pgwc_app().info("    APN AMBR DL ..........: %lu  (Kilo
    //  bits/s)",
    //                          pcef.apn_ambr_dl);
  ...
  bool pgw_config::GetUpNodes(
    ...
    pdncfg.apn " << pdncfg.apn << std::endl; std::cout << "uli " <<
    uli.toString()
