...
void sgwc_app_task(void* args_p) {
  ...
  do {
    ...
    switch (msg->msg_type) {
      ...
      default:
        Logger::sgwc_app().info("no handler for ITTI msg type %d",
                                msg->msg_type);
    }
  } while (true);
}
...
void sgwc_app::handle_itti_msg(itti_s11_create_session_request& csreq) {
  ...
  if ((csreq.teid) && (not sgwc_app_inst->is_s11c_teid_exist(csreq.teid))) {
    Logger::sgwc_app().warn("Received S11 CSReq with dest teid " TEID_FMT
                            " unknown, ignore CSreq",
                            csreq.teid);
    cause_t cause = {
        .cause_value = CONTEXT_NOT_FOUND, .pce = 0, .bce = 0, .cs = 0};
    send_create_session_response_cause(csreq.gtpc_tx_id, csreq.teid,
  ...
  Logger::sgwc_app().debug("sgw_eps_bearer_context: %s!",
                           ebc->toString().c_str());
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
void sgwc_app::send_create_session_response_cause(const uint64_t gtpc_tx_id,
                                                  const teid_t teid,
