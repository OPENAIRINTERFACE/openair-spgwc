...
class GenericMember {
 public:
  ...
  friend inline void swap(GenericMember& a, GenericMember& b)
      RAPIDJSON_NOEXCEPT {
    a.name.Swap(b.name);
...
struct TypeHelper<ValueType, long> {
  ...
  static ValueType& Set(
      ValueType& v, long data, typename ValueType::AllocatorType&) {
    return v.SetInt(data);
  }
};
...
struct TypeHelper<ValueType, unsigned long> {
  ...
  static ValueType& Set(
      ValueType& v, unsigned long data, typename ValueType::AllocatorType&) {
    return v.SetUint(data);
  }
};
#endif
...
struct TypeHelper<ValueType, const typename ValueType::Ch*> {
  ...
  static ValueType& Set(
      ValueType& v, const StringType data,
...
class GenericValue {
  ...
  */
  ...
  SizeType GetStringLength() const {
    RAPIDJSON_ASSERT(IsString());
    return (
        (data_.f.flags & kInlineStrFlag) ? (data_.ss.GetLength()) :
                                           data_.s.length);
  }
 ...
 private:
  ...
  struct Flag {
    ...
    char payload[sizeof(SizeType) * 2 + 6];  // 2 x SizeType + lower 48-bit
                                             // pointer
#elif RAPIDJSON_64BIT
    char payload[sizeof(SizeType) * 2 + sizeof(void*) + 6];  // 6 padding bytes
#else
        char payload[sizeof(SizeType) * 2 + sizeof(void*) + 2];  // 2 padding
                                                                 // bytes
#endif
    uint16_t flags;
  };
...
class GenericObject {
 public:
  ...
  GenericObject MemberReserve(
      SizeType newCapacity, AllocatorType& allocator) const {
...
#if RAPIDJSON_HAS_STDSTRING
  GenericObject AddMember(
      ValueType& name, std::basic_string<Ch>& value,
