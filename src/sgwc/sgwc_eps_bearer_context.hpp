...
public:
  ...
  //ip_address_t            pgw_address_in_use_up;       // The IP address of the P-GW currently used for sending user plane traffic. (For GTP-based S5/S8 only).
  ...
  // replaced by
  fteid_t                 pgw_fteid_s5_s8_up;         // P-GW FTEID for the S5/S8 interface for the user plane. (For GTP-based S5/S8 only).
  ...
  // replaced by
  fteid_t                 sgw_fteid_s5_s8_up;         // S-GW FTEID for the S5/S8 interface for the user plane. (For GTP-based S5/S8 only).
  ...
  // replaced by
  fteid_t                 mme_fteid_s11u;                   // MME FTEID for the S11-u interface (Used by the S-GW). The S11-u interface is used for Control Plane CIoT EPS Optimisation.
...
public:
  ...
  bool update_eps_bearer(const gtpv2c::bearer_context_modified_within_modify_bearer_response& b);
  ...
  // NOT IMPLEMENTED NOW
  ip_address_t         pgw_address_in_use_up;         ///< The IP address of the P-GW currently used for sending user plane traffic. (For PMIP-based S5/S8 only)
...
public:
  ...
  bool find_pdn_connection(const std::string& apn, const pdn_type_t pdn_type, std::shared_ptr<sgw_pdn_connection> &sp);
  bool find_pdn_connection(const ebi_t& ebi, std::shared_ptr<sgw_pdn_connection> &sp);
  ...
  ecgi_field_t                last_known_cell_Id;            // This is the last location of the UE known by the network
